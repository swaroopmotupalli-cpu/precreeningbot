...
> - `tts_ttfb` ≈ 0.2 s ✅ — fast.
> - `llm_ttft` ≈ 1.3 s **measured from a local India laptop over a consumer ISP** — not prefill/caching (prompts are 141–611 tokens, `cached_tokens=0`; caching cannot engage below the model's cache minimum and is a *production* lever once resumes/JDs are large). **Vertex AI in-region is NOT available on this project** (org policy), so the Developer API is the only path and offers no region control. BUT this number is expected to be **pessimistic**: in production the worker runs on **GKE inside Google's network**, where the Developer-API round-trip is materially faster than from a laptop. **The LLM component MUST be re-measured from the production GCP environment before the gate is finalized.**
...
>
> **Revised target (provisional, to be confirmed by a live-mic run from the production GKE environment):** p50 **goal < 1.2 s**, **hard floor < 1.5 s** (do not ship above). Rationale: even fully optimized, semantic-endpointing (~0.4 s) + LLM TTFT (~0.5 s from inside GCP) + TTS (~0.2 s) ≈ **1.1 s** is the realistic single-stream floor for a GA cascade. The local-laptop measurement (~1.9 s live-projected) is **not** representative — the gate must be measured where it ships (GKE). True sub-800 ms would require a speech-to-speech/Live model, explicitly rejected in §12 for being Preview. STT runs concurrently with speech and is not summed into the turn total.
